...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 shadow-lg text-sm">
      ...
      {payload.map(p => <p key={p.dataKey} style={{ color: p.color }}>{p.name}: €{p.value.toLocaleString()}</p>)}
    </div>
  )
}
...
function BtTab({ rezervacije, apartmani }) {
  ...
  const ukupnoNaplaceno = meseci.filter(m => placeni[m]).reduce((s, m) => s + poMesecima[m].reduce((ms, r) => ms + noći(r.dolazak, r.odlazak) * (r.brGostiju || 1) * stopa, 0), 0)
  const ukupnoNeCekanje = meseci.filter(m => !placeni[m]).reduce((s, m) => s + poMesecima[m].reduce((ms, r) => ms + noći(r.dolazak, r.odlazak) * (r.brGostiju || 1) * stopa, 0), 0)
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 col-span-2 sm:col-span-1">
          ...
          {editStopa ? (
            <div className="flex items-center gap-2">
              <input type="number" value={stopa} onChange={e => setStopa(Number(e.target.value))} className="w-24 px-2 py-1 text-sm border border-teal-400 rounded-lg outline-none bg-transparent dark:text-white" autoFocus />
              ...
              {rezMesec.map(r => {
                ...
                return (
                  <div key={r.id} className="flex items-center gap-4 px-5 py-3 text-sm">
                    ...
                    <p className="font-semibold text-slate-700 dark:text-slate-200 whitespace-nowrap w-24 text-right">{(n * g * stopa).toLocaleString()} RSD</p>
                  </div>
                )
              })}
            </div>
      ...
      {tab === 'transakcije' && <>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            { naziv: 'Boravišna taksa', iznos: tranz.filter(t => t.kategorija === 'Boravišna taksa').reduce((s, t) => s + Math.abs(t.iznos), 0), ikona: Receipt, boja: '#f59e0b' },
          ].map(k => {
            const Ik = k.ikona
            return (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 transition-colors">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
            ...
            <div className="flex gap-2">
              <button onClick={() => setIzvestaj(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                <FileText size={14} /> Izveštaj
              </button>
              <button onClick={() => setNoviTrosak(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white rounded-lg hover:opacity-90 transition-opacity" style={{ backgroundColor: '#01696f' }}>
                <Plus size={14} /> Novi trošak
              </button>
            </div>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {tranz.length === 0 && <p className="text-center text-slate-400 py-10 text-sm">Nema transakcija</p>}
            {tranz.map(t => {
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setIzvestaj(false)}>
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: '#01696f20' }}><FileText size={18} style={{ color: '#01696f' }} /></div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select value={repForma.mesec} onChange={e => setRepForma({...repForma, mesec: e.target.value})} className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg outline-none focus:border-teal-500 bg-white dark:bg-slate-800 dark:text-white">
                ...
                <div>
                  ...
                  <select value={repForma.godina} onChange={e => setRepForma({...repForma, godina: e.target.value})} className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg outline-none focus:border-teal-500 bg-white dark:bg-slate-800 dark:text-white">
